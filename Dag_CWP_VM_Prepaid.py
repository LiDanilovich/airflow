...
dag_id = 'CWP_VM_Prepaid'

# variables para triggers sucesivos
branch_id = 'DNA_HoraLímite' 
trigger_dag_id = 'CWP_Compare_DNA_VM_Prepaid'


nro_ejecucion = Variable.get('CWP_VM_PREPAID_EJEC')
...
def choose_branch(task_instance, dag_run):
    ...
    Variable.update(key='CWP_VM_PREPAID_EJEC',value=prox_ejecucion)
   

    dag_runs = DagRun.find(dag_id='CWP_Compare_DNA_VM_Prepaid', state=DagRunState.SUCCESS)
  
    check_hora = control_hora(int(Variable.get('CWP_VM_PREPAID_HORA_LIM').split(':')[0]),int(Variable.get('CWP_VM_PREPAID_HORA_LIM').split(':')[1])) 
    ...
    if len(dag_runs) >= 1:
       ...
       if last.execution_date > actual_ejecucion :
            ...
            valueKey = XCom.get_one(run_id = last.run_id,
                                   ...
                                   dag_id = 'CWP_Compare_DNA_VM_Prepaid',
                                   )
            print('valueKey {}'.format(valueKey))
            if valueKey == '1':
                print('procesar dna')
                return  'trigger_VM_Prepaid_Base'
                ...
                return 'trigger_VM_Prepaid_Resto'
...
with DAG(
    ...
    trigger_VM_Prepaid_Base = TriggerDagRunOperator(
        task_id='trigger_VM_Prepaid_Base',
        trigger_dag_id = 'CWP_VM_Prepaid_Base',
        reset_dag_run = True,
        wait_for_completion = True,
        poke_interval=30 
    )

    trigger_VM_Prepaid_Resto = TriggerDagRunOperator(
        task_id='trigger_VM_Prepaid_Resto',
        trigger_dag_id = 'CWP_VM_Prepaid_Resto',
        reset_dag_run = True,
        wait_for_completion = True,
        poke_interval=30 
    )
    
    trigger_VM_Prepaid_Backups = TriggerDagRunOperator(
        task_id='trigger_VM_Prepaid_Backups',
        trigger_dag_id = 'CWP_VM_Prepaid_Backups',
        reset_dag_run = True,
        wait_for_completion = True,
        poke_interval=30 
    )      
    ...
    trigger_repeat = TriggerDagRunOperator(
        ...
        conf={ 'ejecucion': Variable.get('CWP_VM_PREPAID_EJEC') }
    )
    ...
    branch >>  [trigger_VM_Prepaid_Base, trigger_VM_Prepaid_Resto, sleep_task, jobError ]
    sleep_task >> trigger_repeat
    trigger_VM_Prepaid_Base >> trigger_VM_Prepaid_Resto
    trigger_VM_Prepaid_Resto >> trigger_VM_Prepaid_Backups >> jobEnd
