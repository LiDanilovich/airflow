...
Dag_Name = "CWP_Compare_DNA_VM_Prepaid"

def compara(ti, **context):
    
    hook = OracleHook(oracle_conn_id="Oracle_datalake")
  
    a_csv1 = hook.get_first(sql="SELECT to_char(max(snap_key)) FROM DATALAKE.DL_DNA_PREPAID_BASE_SNAP")
    if a_csv1[0] is None:
       v_csv1 = '0000-00-00'
    else:
       v_csv1 = a_csv1[0]
    ...
    a_csv2 = hook.get_first(sql="SELECT to_char(max(data_dt),'YYYYMMDD') FROM STAGING.VM_SUSCRIBER_CWP where SUSC_MKT='PREPAID' ")
    ...
    if v_csv2 == fecha.strftime('%Y-%m-%d') :
        print('ya estaba actualizado')
        ti.xcom_push(key='newDNA', value='2')
        return 2 
    elif v_csv1 == fecha.strftime('%Y-%m-%d') :
        print("actualizar")
...
with DAG(f'{Dag_Name}',
    ...
    jobStart >>  controlNovedades >> sleep_task >> jobEnd
